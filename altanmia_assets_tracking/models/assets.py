...
class Assets(models.Model):
    ...
    product_id = fields.Many2one('product.product', string='Product', tracking=True,  domain=[('tracking', 'in', ['serial', 'lot'])])
    product_lots_serial = fields.Many2one('stock.lot', string='Lots/Serial Numbers', tracking=True)
    ...
    depreciation_level = fields.Selection([
            ('asset_level', 'Depreciation at asset level'),
            ('parts_level', 'Depreciation at parts level'),
    ...
    def pause(self, pause_date, message=None):
        ...
        result = None
        if self.depreciation_level == 'asset_level':
            result = super(Assets, self).pause(pause_date, message)
        elif self.depreciation_level in ('parts_level', 'parent_level'):
        ...
        # pause parent
        if self.parent:
            self.parent.pause(pause_date, message)

        # pause parts
        for part in self.parts:
            part.pause(pause_date, message)
